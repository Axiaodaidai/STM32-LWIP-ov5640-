...
//80 0000
//����ͷͼ�񻺳���
__attribute__ ((at(0xD0100000))) uint16_t cam_buff0[480*272];
__attribute__ ((at(0xD0300000))) uint16_t cam_buff1[480*272];
//��ǰʹ�õ��ڴ��
int cur_index = 0;
...
//    }
    if(cur_index == 0)//0--���õڶ����ڴ棬ʹ�õ�һ���ڴ�
    {
      ...
      /* one-shot capture handoff: cam_buff0 is the completed stable frame */
      if (g_camtcp_cap_request)
      {
        g_camtcp_cap_request = 0;
        g_camtcp_cap_ptr = (uint8_t*)cam_buff0;
        g_camtcp_cap_len = (uint32_t)(cam_mode.cam_out_width * cam_mode.cam_out_height * 2);
        g_camtcp_cap_seq++;
        g_camtcp_cap_ready = 1;
      }
    ...
    else//1--���õ�һ���ڴ棬ʹ�õڶ����ڴ�
    {
      ...
      /* one-shot capture handoff: cam_buff1 is the completed stable frame */
      if (g_camtcp_cap_request)
      {
        g_camtcp_cap_request = 0;
        g_camtcp_cap_ptr = (uint8_t*)cam_buff1;
        g_camtcp_cap_len = (uint32_t)(cam_mode.cam_out_width * cam_mode.cam_out_height * 2);
        g_camtcp_cap_seq++;
        g_camtcp_cap_ready = 1;
      }
