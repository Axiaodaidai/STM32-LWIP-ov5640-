/**
  ...
  * @version V1.1
  * @date    2015-xx-xx
  * @brief   LCD应用函数接口，底层RGB565格式，顶层RGB1555格式。
						摄像头显示在底层，文字显示在顶层。摄像头显示部分的液晶层不支持RGB888格式。
  ******************************************************************************
  * @attention
  *
...
void LCD_LayerInit(void)
{
  ...
  /* 配置本层的窗口边界，注意这些参数是包含HBP HSW VBP VSW的 */    
	...
	LTDC_Layer_InitStruct.LTDC_VerticalStop = VSW+VBP+LCD_PIXEL_HEIGHT-4-1;
 ...
 */
 ...
 LTDC_Layer_InitStruct.LTDC_PixelFormat = LTDC_Pixelformat_ARGB1555;
  ...
  /* Start Address configuration : the LCD Frame buffer is defined on SDRAM w/ Offset */
 LTDC_Layer_InitStruct.LTDC_CFBStartAdress = Addr + BUFFER_OFFSET;
...
void LCD_CamLayerInit(uint32_t Addr)
{
 LTDC_Layer_InitTypeDef LTDC_Layer_InitStruct;
...
void LCD_SetCamLayer(uint32_t Addr)
{
   CurrentFrameBuffer = Addr;
   CurrentLayer = LCD_BACKGROUND_LAYER;
}
